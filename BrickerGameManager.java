...
//import brick_strategies.CollisionStrategyGeneratorLevel2 ;
//
...
//    private UserInputListener inputListener;
//    private boolean gameOver = false;
//    private Lives lives; // Keep a reference to the Lives object
//
//    // Utility interfaces
//    private AddGameObject addGameObjectCommand;
//    private RemoveGameObject removeGameObjectCommand;
...
//        GameObject[] walls = new GameObject[] {
//                new GameObject(new Vector2(0, -WALL_WIDTH),
//                        new Vector2(windowDims.x(), WALL_WIDTH), wallImage),
//                new GameObject(new Vector2(-WALL_WIDTH, 0),
//                        new Vector2(WALL_WIDTH, windowDims.y()), wallImage),
//                new GameObject(new Vector2(windowDims.x(), 0),
//                        new Vector2(WALL_WIDTH, windowDims.y()), wallImage)
//        };
//
...
//        if (ball.getCenter().y() > windowDims.y()) {
//            playerLives--;
//            // Update lives UI to reflect the reduced number of lives
//            lives.updateLives(); // Ensure this method exists in the Lives class to refresh the display
//
//            if (playerLives <= 0) {
//                loseGame();
//            } else {
//                // Just reposition ball and paddle, do not reset bricks or entire game
...
//        // Move the ball back to the center and give it a new velocity
//        ball.setCenter(windowDims.mult(0.5f));
//        ball.setVelocity(new Vector2(0, 200));
//
//        // Reposition the paddle back to bottom center
//        paddle.setCenter(new Vector2(windowDims.x() / 2, windowDims.y() - 50));
//    }
//
//    public static void main(String[] args) {
//        new BrickerGameManager("Bricker Game", DEFAULT_WINDOW_SIZE).run();
...
public class BrickerGameManager extends GameManager {
    ...
    public void initializeGame(ImageReader imageReader, SoundReader soundReader,
        ...
        createBalls(imageReader, soundReader, inputListener);
        createPaddle(imageReader, inputListener);
        AddLife addLifeCommand = new AddLifeImpl(() -> playerLives++);

        CollisionStrategyGenerator strategyGenerator = new CollisionStrategyGeneratorLevel2(
                inputListener,
                imageReader,
                windowDims,
                addGameObjectCommand,
                removeGameObjectCommand,
                soundReader,
                null,
                addLifeCommand
        );
    ...
    private void createBalls(ImageReader imageReader, SoundReader soundReader,
        ...
        for (int i = 0; i < DEFAULT_BALLS_NUMBER; i++) {
            ball = new Ball(windowDims, imageReader, soundReader, inputListener);
            addGameObjectCommand.add(ball, Layer.DEFAULT);
        }
    }

    private void createPaddle(ImageReader imageReader, UserInputListener inputListener) {
        paddle = new Paddle(windowDims.x(), windowDims.y(), inputListener, imageReader);
        addGameObjectCommand.add(paddle, Layer.DEFAULT);
    }
    ...
    private void createBricks(ImageReader imageReader, CollisionStrategyGenerator strategyGenerator) {
        ...
        for (int row = 0; row < bricksPerColumn; row++) {
            for (int col = 0; col < bricksPerRow; col++) {
                ...
                Brick brick = new Brick(position, brickSize, imageReader, strategyGenerator.generateStrategy());
                addGameObjectCommand.add(brick, Layer.STATIC_OBJECTS);
                bricks++;
            }
        }
